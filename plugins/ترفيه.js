let handler = m => m 
 ...
 if (/^حد موجود|حد حنا$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `👹ابليس`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
 ...
     conn.reply(m.chat, `😳🏳️‍🌈`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
  
 } 
     
 if (/^افضل انمي$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*كونان عمك*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 

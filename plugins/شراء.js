const items = ['نقود', 'اكسبي']
let confirmation = {}

async function handler(m, { conn, args, usedPrefix, command }) {
    if (confirmation[m.sender]) return m.reply('أنت تقوم بعحويل')
    let user = global.db.data.users[m.sender]
    const item = items.filter(v => v in user && typeof user[v] == 'number')
    let lol = `😕 الاستخدام الصحيح للأمرر 
*${usedPrefix + command}*  [النوع] [كميه] [@user]

📌 مثال : 
*${usedPrefix + command}* exp 65 @${m.sender.split('@')[0]}

📍 الأشياء القابلة لتحويل
┌──────────────
▢ *نقود* = نقودك 🪙
▢ *اكسبي* = تَجْرِبة 🆙
└──────────────
`.trim()
    ...
    if (!(who in global.db.data.users)) return m.reply(`✳️ المستخدم غير موجود في قاعدة البيانات الخاصة بي`)
    if (user[type] * 1 < count) return m.reply(`✳️  *${type}* لا يكفي لنقل`)
    let confirm = `
